...
do is=1,nspecies
  ...
  if (nrmt(is) < 20) then
    write(*,*)
    write(*,'("Error(readspecies): nrmt too small : ",I0)') nrmt(is)
    write(*,'(" for species ",I0)') is
    write(*,*)
    stop
  end if
...
! reduce the minimum radial mesh point by the same factor
  ...
  if ((nstsp(is) < 1).or.(nstsp(is) > maxstsp)) then
    write(*,*)
    write(*,'("Error(readspecies): nstsp out of range : ",I0)') nstsp(is)
    write(*,'(" for species ",I0)') is
    write(*,*)
    stop
  end if
  do ist=1,nstsp(is)
    ...
    if (nsp(ist,is) < 1) then
      write(*,*)
      write(*,'("Error(readspecies): nsp < 1 : ",I0)') nsp(ist,is)
      write(*,'(" for species ",I0)') is
      write(*,'(" and state ",I0)') ist
      write(*,*)
      stop
    end if
    if (lsp(ist,is) < 0) then
      write(*,*)
      write(*,'("Error(readspecies): lsp < 0 : ",I0)') lsp(ist,is)
      write(*,'(" for species ",I0)') is
      write(*,'(" and state ",I0)') ist
      write(*,*)
      stop
    ...
    if (ksp(ist,is) < 1) then
      write(*,*)
      write(*,'("Error(readspecies): ksp < 1 : ",I0)') ksp(ist,is)
      write(*,'(" for species ",I0)') is
      write(*,'(" and state ",I0)') ist
      write(*,*)
      stop
  ...
  if (apword(0,is) < 1) then
    write(*,*)
    write(*,'("Error(readspecies): apword < 1 : ",I0)') apword(0,is)
    write(*,'(" for species ",I0)') is
    write(*,*)
    stop
  end if
  if (apword(0,is) > maxapword) then
    write(*,*)
    write(*,'("Error(readspecies): apword too large : ",I0)') apword(0,is)
    write(*,'(" for species ",I0)') is
...
! set the APW orders for l>0
  ...
  do io=1,apword(0,is)
    ...
    if (apwdm(io,0,is) < 0) then
      write(*,*)
      write(*,'("Error(readspecies): apwdm < 0 : ",I0)') apwdm(io,0,is)
      write(*,'(" for species ",I0)') is
...
! set the APW linearisation energies, derivative orders and variability for l>0
  ...
  if (nlx < 0) then
    write(*,*)
    write(*,'("Error(readspecies): nlx < 0 : ",I0)') nlx
    write(*,'(" for species ",I0)') is
    write(*,*)
    stop
  end if
  do ilx=1,nlx
    ...
    if (lx < 0) then
      write(*,*)
      write(*,'("Error(readspecies): lx < 0 : ",I0)') lx
      write(*,'(" for species ",I0)') is
      write(*,'(" and exception number ",I0)') ilx
      write(*,*)
      stop
    end if
    if (lx > lmaxapw) then
      write(*,*)
      write(*,'("Error(readspecies): lx > lmaxapw : ",I0)') lx
      write(*,'(" for species ",I0)') is
      write(*,'(" and exception number ",I0)') ilx
      write(*,*)
      stop
    ...
    if (apword(lx,is) < 1) then
      write(*,*)
      write(*,'("Error(readspecies): apword < 1 : ",I0)') apword(lx,is)
      write(*,'(" for species ",I0)') is
      write(*,'(" and exception number ",I0)') ilx
      write(*,*)
      stop
    ...
    if (apword(lx,is) > maxapword) then
      write(*,*)
      write(*,'("Error(readspecies): apword too large : ",I0)') apword(lx,is)
      write(*,'(" for species ",I0)') is
      write(*,'(" and exception number ",I0)') ilx
      write(*,'("Adjust maxapword in modmain and recompile code")')
      write(*,*)
    ...
    do io=1,apword(lx,is)
      ...
      if (apwdm(io,lx,is) < 0) then
        write(*,*)
        write(*,'("Error(readspecies): apwdm < 0 : ",I0)') apwdm(io,lx,is)
        write(*,'(" for species ",I0)') is
        write(*,'(" exception number ",I0)') ilx
        write(*,'(" and order ",I0)') io
        write(*,*)
        stop
      end if
...
! add excess order to APW functions if required
  ...
  if (nlorb(is) < 0) then
    write(*,*)
    write(*,'("Error(readspecies): nlorb < 0 : ",I0)') nlorb(is)
    write(*,'(" for species ",I0)') is
    write(*,*)
    stop
  end if
  if (nlorb(is) > maxlorb) then
    write(*,*)
    write(*,'("Error(readspecies): nlorb too large : ",I0)') nlorb(is)
    write(*,'(" for species ",I0)') is
  ...
  do ilo=1,nlorb(is)
    ...
    if (lorbl(ilo,is) < 0) then
      write(*,*)
      write(*,'("Error(readspecies): lorbl < 0 : ",I0)') lorbl(ilo,is)
      write(*,'(" for species ",I0)') is
    ...
    if (lorbl(ilo,is) > lmaxo) then
      write(*,*)
      write(*,'("Error(readspecies): lorbl > lmaxo :",2(X,I0))') lorbl(ilo,is),&
       lmaxo
      write(*,'(" for species ",I0)') is
      write(*,'(" and local-orbital ",I0)') ilo
      write(*,*)
    ...
    if (lorbord(ilo,is) < 2) then
      write(*,*)
      write(*,'("Error(readspecies): lorbord < 2 : ",I0)') lorbord(ilo,is)
      write(*,'(" for species ",I0)') is
      write(*,'(" and local-orbital ",I0)') ilo
      write(*,*)
      stop
    ...
    if (lorbord(ilo,is) > maxlorbord) then
      write(*,*)
      write(*,'("Error(readspecies): lorbord too large : ",I0)') lorbord(ilo,is)
      write(*,'(" for species ",I0)') is
      write(*,'(" and local-orbital ",I0)') ilo
    ...
    do io=1,lorbord(ilo,is)
      ...
      if (lorbdm(io,ilo,is) < 0) then
        write(*,*)
        write(*,'("Error(readspecies): lorbdm < 0 : ",I0)') lorbdm(io,ilo,is)
        write(*,'(" for species ",I0)') is
        write(*,'(" local-orbital ",I0)') ilo
        write(*,'(" and order ",I0)') io
        write(*,*)
        stop
      end if
