...
!   zvclmt : muffin-tin charge density on input and Coulomb potential on output
!            (inout,complex(*))
! !DESCRIPTION:
...
!   Combined density and potential arrays, January 2026 (JKD)
!EOP
!BOC
implicit none
! arguments
...
do l=0,lmaxi
  ...
  do lm=l**2+1,(l+1)**2
    do ir=1,nri
      i=lm+lmmaxi*(ir-1)
      f1(ir)=rl(ir,l1)*zvclmt(i)
      f2(ir)=rl(ir,l2)*zvclmt(i)
    end do
    i0=lm+npi
    do ir=iro,nr
      i=i0+lmmaxo*(ir-iro)
