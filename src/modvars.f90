...
if (present(iva).or.present(rva).or.present(zva).or.present(sva)) then
  if (.not.present(nv)) then
    ...
    if (nv < 0) then
      write(*,*)
      write(*,'("Error(writevars): nv < 0 : ",I0)') nv
      write(*,*)
      stop
    end if
  end if
end if
open(95,file='VARIABLES.OUT',form='FORMATTED',action='WRITE',position='APPEND')
write(95,*)
...
if (present(iv)) then
  ...
  write(95,'(2I8)') 3,1
  write(95,'(2G24.14)') dble(zv),aimag(zv)
  ...
  write(95,'(2I8)') 2,nv
  do i=1,nv
    write(95,'(G24.14)') rva(i)
  end do
else if (present(zva)) then
  write(95,'(2I8)') 3,nv
  do i=1,nv
    write(95,'(2G24.14)') dble(zva(i)),aimag(zva(i))
  end do
