...
! constant potential matrix elements for fixorb               !LZ mod
  if (allocated(vmatmt_const)) deallocate(vmatmt_const)       !LZ mod
  allocate(vmatmt_const(lmmaxdm,nspinor,lmmaxdm,nspinor))     !LZ mod
! zero the potential matrix
  vmatmt(:,:,:,:,:)=0.d0
! zero the constant potential matrix                          !LZ mod
